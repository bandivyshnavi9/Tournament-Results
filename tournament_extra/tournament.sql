...
CREATE TABLE match(
                   winnerid int REFERENCES players (playerid) ON DELETE CASCADE,
                   loserid int REFERENCES players (playerid) ON DELETE CASCADE,
                   matchid SERIAL PRIMARY KEY, CHECK (winnerid <> loserid));


-- The playerscount view is used to return the distinct number of players

CREATE VIEW playerscount AS (select count(*) as pcount from players where playerid != -1);
...
) as matchesplayed where players.playerid = matchesplayed.playerid and players.playerid != -1 order by matchesplayed.wins DESC);
